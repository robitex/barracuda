...
pdfinfo={
    ...
    Keywords={Barcode EAN Code128 ITF14 Lua}
}}
...
\code{texdoc} utility in a terminal session, typing \code{luatex} as the sole
argument.
\end{description}
...
Common options and parameters are defined 

For each barcode symbologies this section reports parameters and optional
methods of it.

\subsection{Code39}
\label{secCode39}

\code{Code39} is one of the oldest symbologies ever created. It does not include
...
The Barcode class is completely indipendent from the output driver and viceversa.
...
multiple of the fixed lenght called \code{mod}, defined by the array and fugure
showed below:
\begin{Verbatim}
-- {     x1,    w1,      x2,    w2,     x3,     w3}
...
For clearness, to the drawing were added gray vertical grid stepping one module
and white dashed lines at every bar axis.
...
The class \code{Vbar} of module \code{libgeo} has several costructors one of
which is \code{from\_int()}. Its arguments are the multipler integer
\code{ngen}, the module lenght \code{mod} and the optional boolean flag
...
A \code{VbarQueue} code example is the following (in the drawing some graphical
aid has been added: a vertical grid marks the module wide step and light colored
bars mark the space added between two \code{Vbar}s):
\begin{Verbatim}
% !TeX program = LuaTeX
\newbox\mybox
\directlua{
...
\begin{tikzpicture}
\fill[orange!50!white] (16mm, -2mm) rectangle (18mm, 28mm); %
\fill[orange!50!white] (28mm, -2mm) rectangle (30mm, 28mm); %
\fill[orange!50!white] (48mm, -2mm) rectangle (50mm, 28mm); %
\foreach \i in {0,2,...,68}
\draw[help lines] (\i mm, -2mm) -- (\i mm, 28mm);
