...
In the phase \textbf{a} a barcode symbols is created, then in phase \textbf{b}
a canvas object is filled with graphic elements, and finally in the phase
\textbf{c} the canvas is send to the driver output channel.
...
instance, phase \textbf{b} can be fuse with \textbf{c}, because a \code{canvas}
object include internally a reference to the driver object:
\begin{Verbatim}
-- phase b + c
...
An high level method provide as we have been seen before, a way to unify phases:
\begin{Verbatim}
...
\code{debug\_bbox\_on} & boolean & \code{false} & Draw symbol bounding box with a thin dashed line\\
\bottomrule
\end{tabular}
\end{center}
...
A more abstract way to write a \code{ga} stream is provided by the \code{gacanvas}
class of the module \code{libgeo}. Every operation has been mapped to a method
named \code{encode\_<opcode>()}:
\begin{Verbatim}
% !TeX program = LuaTeX
\newbox\mybox
\directlua{
...
until that instruction. The \code{<set\_bbox>} operation imposes a
bounding box in comparison to the current figure one.
