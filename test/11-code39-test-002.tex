...
local _, err = symbol:append_graphic(canvas)
assert(not err, err)
...
local s2, err = c39:from_string("RTFG746", {text_pos="top"}); assert(s2, err)

local canvas = barracuda:new_canvas()
local _, err = s1:append_graphic(canvas)
assert(not err, err)

local h2 = s2:get_param("height")
local _, err = s2:append_graphic(canvas, 0.0, h2 + tex.sp "5pt")
assert(not err, err)
local drv, err = barracuda:load_driver("native")
assert(not err, err)
drv:ga_to_hbox(canvas, "mybox")
...
local canvas = barracuda:new_canvas()
local _, err = s1:append_graphic(canvas); assert(not err, err)

local h = s1:get_param("height")
c39:set_param("height", 2*h)
c39:set_param {text_pos = "top"}

local _, err = s1:append_graphic(canvas, 0.0, h + tex.sp "5pt")
assert(not err, err)
...
local ok, err = s1:set_param{
    height = h/2,
    text_enabled = true,
    text_pos = "bottom-spaced",
    text_star = true,
    ay = 1,
}; assert(ok, err)
local _, err = s1:append_graphic(canvas, 0.0, - tex.sp "5pt"); assert(not err, err)
